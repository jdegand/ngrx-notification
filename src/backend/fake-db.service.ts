...
export class FakeDBService extends ComponentStore<AppState> {
    ...
    addTeacher = this.updater((state, teacher: Teacher): AppState => ({
        ...state,
        teachers: [...state.teachers, teacher],
    }));

    updateTeacher = this.updater((state, teacher: Teacher): AppState => ({
        ...state,
        teachers: state.teachers.map((t) => (t.id === teacher.id ? teacher : t)),
    }));

    addStudent = this.updater((state, student: Student): AppState => ({
        ...state,
        students: [...state.students, student],
    }));

    updateSudent = this.updater((state, student: Student): AppState => ({
        ...state,
        students: state.students.map((t) => (t.id === student.id ? student : t)),
    }));

    addSchool = this.updater((state, school: School): AppState => ({
        ...state,
        schools: [...state.schools, school],
    }));

    updateSchool = this.updater((state, school: School): AppState => ({
        ...state,
